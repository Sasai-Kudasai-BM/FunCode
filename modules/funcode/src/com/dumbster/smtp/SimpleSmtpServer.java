...
public final class SimpleSmtpServer implements AutoCloseable {
	...
	public void stop() {
		...
		try {
			...
			System.err.println("trouble closing the server socket" + e);
		}
		// and block until worker is finished
		try {
			workerThread.join(STOP_TIMEOUT);
		} catch (InterruptedException e) {
			System.err.println("interrupted when waiting for worker thread to finish" + e);
		}
	}
	...
	private void performWork() {
		try {
			...
			if (!stopped) {
				System.err.println("hit exception when running server" + e);
				try {
					serverSocket.close();
				} catch (IOException ex) {
					System.err.println("and one when closing the port" + ex);
				}
			}
		}
	}
