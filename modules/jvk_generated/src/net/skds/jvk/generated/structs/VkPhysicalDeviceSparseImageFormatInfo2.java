package net.skds.jvk.generated.structs;

import java.lang.foreign.Arena;
import java.lang.foreign.MemoryLayout;
import java.lang.foreign.MemorySegment;
import net.skds.jvk.StructConstructor;
import net.skds.jvk.VkStructArray;
import net.skds.jvk.annotation.NativeType;
import net.skds.lib2.annotations.AutoGenerated;
import net.skds.lib2.natives.struct.CStruct;

import static java.lang.foreign.ValueLayout.*;
import static net.skds.lib2.natives.SafeLinker.*;
import static java.lang.foreign.MemoryLayout.*;

/**
*<pre>
* C definition:
* struct VkPhysicalDeviceSparseImageFormatInfo2 {
* 	VkStructureType sType;
* 	// pad (4)
* 	void* pNext;
* 	VkFormat format;
* 	VkImageType type;
* 	VkSampleCountFlagBits samples;
* 	VkImageUsageFlags usage;
* 	VkImageTiling tiling;
* 	// pad (4)
* };
*</pre>
**/
@AutoGenerated
public final class VkPhysicalDeviceSparseImageFormatInfo2 extends CStruct {
	
	public static final MemoryLayout MEMORY_LAYOUT = structLayout(
			JAVA_INT,
			paddingLayout(4),
			JAVA_LONG,
			JAVA_INT,
			JAVA_INT,
			JAVA_INT,
			JAVA_INT,
			JAVA_INT,
			paddingLayout(4)
	);
	private static final int S_TYPE_OFFSET = 0;
	private static final int P_NEXT_OFFSET = 8;
	private static final int FORMAT_OFFSET = 16;
	private static final int TYPE_OFFSET = 20;
	private static final int SAMPLES_OFFSET = 24;
	private static final int USAGE_OFFSET = 28;
	private static final int TILING_OFFSET = 32;
	
	public VkPhysicalDeviceSparseImageFormatInfo2() {
		this.segment = Arena.ofAuto().allocate(MEMORY_LAYOUT.byteSize());
	}
	
	public VkPhysicalDeviceSparseImageFormatInfo2(Arena arena) {
		this.segment = arena.allocate(MEMORY_LAYOUT.byteSize());
	}
	
	public VkPhysicalDeviceSparseImageFormatInfo2(MemorySegment segment, long offset) {
		super(segment, offset);
	}
	
	
	/** values = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_SPARSE_IMAGE_FORMAT_INFO_2 **/
	@NativeType("VkStructureType")
	public int sType() {
		return (int) INT_HANDLE.get(this.segment, this.offset + S_TYPE_OFFSET);
	}
	
	/** values = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_SPARSE_IMAGE_FORMAT_INFO_2 **/
	@NativeType("VkStructureType")
	public VkPhysicalDeviceSparseImageFormatInfo2 sType(int value) {
		INT_HANDLE.set(this.segment, this.offset + S_TYPE_OFFSET, value);
		return this;
	}
	
	/** values = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_SPARSE_IMAGE_FORMAT_INFO_2 **/
	@NativeType("VkStructureType")
	public VkPhysicalDeviceSparseImageFormatInfo2 sType$Default() {
		INT_HANDLE.set(this.segment, this.offset + S_TYPE_OFFSET, 1000059008);
		return this;
	}
	
	/** optional **/
	@NativeType("void*")
	public long pNext() {
		return (long) LONG_HANDLE.get(this.segment, this.offset + P_NEXT_OFFSET);
	}
	
	/** optional **/
	@NativeType("void*")
	public VkPhysicalDeviceSparseImageFormatInfo2 pNext(long value) {
		LONG_HANDLE.set(this.segment, this.offset + P_NEXT_OFFSET, value);
		return this;
	}
	
	@NativeType("VkFormat")
	public int format() {
		return (int) INT_HANDLE.get(this.segment, this.offset + FORMAT_OFFSET);
	}
	
	@NativeType("VkFormat")
	public VkPhysicalDeviceSparseImageFormatInfo2 format(int value) {
		INT_HANDLE.set(this.segment, this.offset + FORMAT_OFFSET, value);
		return this;
	}
	
	@NativeType("VkImageType")
	public int type() {
		return (int) INT_HANDLE.get(this.segment, this.offset + TYPE_OFFSET);
	}
	
	@NativeType("VkImageType")
	public VkPhysicalDeviceSparseImageFormatInfo2 type(int value) {
		INT_HANDLE.set(this.segment, this.offset + TYPE_OFFSET, value);
		return this;
	}
	
	@NativeType("VkSampleCountFlagBits")
	public int samples() {
		return (int) INT_HANDLE.get(this.segment, this.offset + SAMPLES_OFFSET);
	}
	
	@NativeType("VkSampleCountFlagBits")
	public VkPhysicalDeviceSparseImageFormatInfo2 samples(int value) {
		INT_HANDLE.set(this.segment, this.offset + SAMPLES_OFFSET, value);
		return this;
	}
	
	@NativeType("VkImageUsageFlags")
	public int usage() {
		return (int) INT_HANDLE.get(this.segment, this.offset + USAGE_OFFSET);
	}
	
	@NativeType("VkImageUsageFlags")
	public VkPhysicalDeviceSparseImageFormatInfo2 usage(int value) {
		INT_HANDLE.set(this.segment, this.offset + USAGE_OFFSET, value);
		return this;
	}
	
	@NativeType("VkImageTiling")
	public int tiling() {
		return (int) INT_HANDLE.get(this.segment, this.offset + TILING_OFFSET);
	}
	
	@NativeType("VkImageTiling")
	public VkPhysicalDeviceSparseImageFormatInfo2 tiling(int value) {
		INT_HANDLE.set(this.segment, this.offset + TILING_OFFSET, value);
		return this;
	}
	
	
	@AutoGenerated
	public static final class Array extends VkStructArray<VkPhysicalDeviceSparseImageFormatInfo2> {
		
		private static final StructConstructor<VkPhysicalDeviceSparseImageFormatInfo2> CONSTRUCTOR = VkPhysicalDeviceSparseImageFormatInfo2::new;
		
		public Array(int size) {
			super(Arena.ofAuto(), new VkPhysicalDeviceSparseImageFormatInfo2[size], CONSTRUCTOR, MEMORY_LAYOUT);
		}
		
		public Array(Arena arena, int size) {
			super(arena, new VkPhysicalDeviceSparseImageFormatInfo2[size], CONSTRUCTOR, MEMORY_LAYOUT);
		}
		
		public Array(MemorySegment segment, long offset, int size) {
			super(segment, offset, new VkPhysicalDeviceSparseImageFormatInfo2[size], CONSTRUCTOR, MEMORY_LAYOUT);
		}
		
	}
	
	@Override
	public int getAlignment() {
		return (int) MEMORY_LAYOUT.byteAlignment();
	}
	
	@Override
	public int getSize() {
		return (int) MEMORY_LAYOUT.byteSize();
	}
	
}