package net.skds.jvk.generated.extensions;

import java.lang.invoke.MethodHandle;
import net.skds.jvk.VkDefinitions;
import net.skds.jvk.annotation.NativeType;
import net.skds.lib2.annotations.AutoGenerated;

import static net.skds.lib2.natives.SafeLinker.*;

@AutoGenerated
public final class VkKhrVideoDecodeQueue {
	
	public static final int VK_STRUCTURE_TYPE_VIDEO_DECODE_INFO_KHR = 0;
	public static final int VK_STRUCTURE_TYPE_VIDEO_DECODE_CAPABILITIES_KHR = 1;
	public static final int VK_QUEUE_VIDEO_DECODE_BIT_KHR = 32;
	public static final long VK_PIPELINE_STAGE_2_VIDEO_DECODE_BIT_KHR = 67108864L;
	public static final long VK_ACCESS_2_VIDEO_DECODE_READ_BIT_KHR = 34359738368L;
	public static final long VK_ACCESS_2_VIDEO_DECODE_WRITE_BIT_KHR = 68719476736L;
	public static final int VK_BUFFER_USAGE_VIDEO_DECODE_SRC_BIT_KHR = 8192;
	public static final int VK_BUFFER_USAGE_VIDEO_DECODE_DST_BIT_KHR = 16384;
	public static final int VK_IMAGE_USAGE_VIDEO_DECODE_DST_BIT_KHR = 1024;
	public static final int VK_IMAGE_USAGE_VIDEO_DECODE_SRC_BIT_KHR = 2048;
	public static final int VK_IMAGE_USAGE_VIDEO_DECODE_DPB_BIT_KHR = 4096;
	public static final int VK_FORMAT_FEATURE_VIDEO_DECODE_OUTPUT_BIT_KHR = 33554432;
	public static final int VK_FORMAT_FEATURE_VIDEO_DECODE_DPB_BIT_KHR = 67108864;
	public static final int VK_IMAGE_LAYOUT_VIDEO_DECODE_DST_KHR = 0;
	public static final int VK_IMAGE_LAYOUT_VIDEO_DECODE_SRC_KHR = 1;
	public static final int VK_IMAGE_LAYOUT_VIDEO_DECODE_DPB_KHR = 2;
	private static final MethodHandle vkCmdDecodeVideoKHR = createHandle(VkDefinitions.LIBRARY_LOOKUP, "vkCmdDecodeVideoKHR", VOID, LONG, LONG);
	
	@NativeType("void")
	public static void vkCmdDecodeVideoKHR(@NativeType("VkCommandBuffer*") long commandBuffer, @NativeType("VkVideoDecodeInfoKHR*") long pFrameInfo) {
		try {
			vkCmdDecodeVideoKHR.invokeExact(commandBuffer, pFrameInfo);
		} catch (Throwable e) {
			throw new Error(e);
		}
	}
	
	
	public static final long VK_FORMAT_FEATURE_2_VIDEO_DECODE_OUTPUT_BIT_KHR = 33554432L;
	public static final long VK_FORMAT_FEATURE_2_VIDEO_DECODE_DPB_BIT_KHR = 67108864L;
}